    ...
    if request.user.is_authenticated:
        if request.user.userprofile.active_team_id:
            ...
            untracked_entries = Entry.objects.filter(
                team=team, created_by=request.user, minutes=0, is_tracked=False
            )

            if untracked_entries:
                active_entry = untracked_entries.first()
                active_entry.seconds_since = int(
                    (
                        datetime.now(timezone.utc) - active_entry.created_at
                    ).total_seconds()
                )

                return {
                    "active_entry_seconds": active_entry.seconds_since,
